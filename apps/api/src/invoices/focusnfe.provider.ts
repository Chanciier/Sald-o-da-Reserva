  ...
  private buildNfePayload(payload: InvoicePayload, isoDt: string): Record<string, unknown> {
    ...
    return {
      ...
      items: items.map((item, idx) => ({
        ...
        icms_origem: item.icmsOrigem ?? 0,
        icms_situacao_tributaria: item.icmsSituacaoTributaria || '102',
        incluir_no_total: 1,
      ...
      ...(payload.carrierName && payload.carrierName !== 'N/A'
        ? { transportador_nome: payload.carrierName }
        : {}),
