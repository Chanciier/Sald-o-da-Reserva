...
// Vendedores com a seção "Clientes" liberada usam esta listagem somente-
// leitura em vez de /admin/rbac/users (que é restrita a ADMIN e permite
// trocar o perfil de outros usuários).
async function fetchClientes(token: string, page: number, search: string) {
  const params = new URLSearchParams({ page: String(page) });
  if (search) params.set('search', search);
  const res = await fetch(`${BASE}/api/v1/admin/rbac/clientes?${params}`, {
    headers: { Authorization: `Bearer ${token}` },
    cache: 'no-store',
  });
  const data = await res.json();
...
export default function AdminUsuarios() {
  ...
  const { data, isLoading, isFetching, refetch } = useQuery({
    queryKey: ['admin-users', page, roleFilter, search, isVendedor],
    queryFn: () =>
      isVendedor
        ? fetchClientes(token!, page, search)
        : fetchUsers(token!, page, roleFilter, search),
    enabled: !!token,
  });
    ...
    <SectionGate section="CLIENTES">
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold">{isVendedor ? 'Clientes' : 'Usuários'}</h1>
          <button
            onClick={() => refetch()}
            disabled={isFetching}
              ...
              <table className="w-full text-sm">
                <thead className="border-b bg-muted/40">
                  <tr className="text-left text-xs text-muted-foreground">
                    ...
                    {!isVendedor && <th className="px-4 py-3 font-medium">Ações</th>}
                  </tr>
                </thead>
